...
%DIF DEL 20150312_QC_Paper.tex   Fri Mar 13 09:56:01 2015
%DIF ADD 20150325_QC_Paper.tex   Sat Mar 28 07:20:03 2015
...
\end{equation}
\ni where $\mathcal{P}_h$ is the set of all pairs of residuals that are $h$ hours apart (or within some window)\DIFdelbegin \DIFdel{, and $\widehat{\epsilon_i}$ is the }\DIFdelend \DIFaddbegin \DIFadd{; $|\mathcal{P}_h|$ is the number of pairs of residuals in the set $\mathcal{P}_h$;  $\hat{\epsilon}_i$ is the }\DIFaddend observed residual from Equation~(\ref{eq:GAM})\DIFaddbegin \DIFadd{; $\bar\epsilon$ is the average of the residuals; and $s^2_{\hat\epsilon}$ is the standard deviation of the residuals}\DIFaddend .  For an AR(1) model, we need only estimate $\phi(\cdot)$ at $h=12$ hours, and we use a window of 5\% of 12 hours, or 0.6 hours.
...
The threshold for determining when a change point has occurred is based on the sampling distribution of Equation~(\ref{eq:Hom}).  We first note that the SNHT \DIFdelbegin \DIFdel{algorithms generally make the assumption that the data is independent and approximately }\DIFdelend \DIFaddbegin \DIFadd{algorithm generally makes the assumptions that the observations are independent and }\DIFaddend normal with some fixed mean\DIFdelbegin \DIFdel{, and under this assumption, the statistic follows a chi-squared distribution with two degrees of freedom, i.  e., $T_i\sim \chi^2_2$. }\textbf{%DIFDELCMD < [%%%
\DIFdel{Josh--isn't this a $\chi^2$ with 2 df's?}%DIFDELCMD < ]%%%
} %DIFAUXCMD
\textbf{%DIFDELCMD < [%%%
\DIFdel{Josh: I still need to check this reference, but I'm pretty sure the Alexandersson paper states this}%DIFDELCMD < ]%%%
}  %DIFAUXCMD
\DIFdel{This assumption is unreasonable with }\DIFdelend \DIFaddbegin \DIFadd{.  Under these assumptions, 
}\begin{equation*}\DIFadd{
	\frac{N}{2 \sigma^2}\left( (\bar{X}_{L,i}-\bar{X}_{R,i})^2\right) \sim \chi^2_1,
}\end{equation*}
\DIFadd{\ni where $\sigma$ is the variance of the $X_i$.  If we replace $\sigma^2$ with $s_i^2$ we get our test statistic, and so $T_i$ is approximately $\chi^2_1$.  However, these assumptions do not all hold for }\DIFaddend radiosonde temperature data, as temperatures follow seasonal trends, which induce a strong dependence among the observations.  Thus, homogenization will likely be improved if the seasonal components of the data are removed.  We use a model-based approach to removing the seasonality; however, we  \DIFdelbegin \DIFdel{must }\DIFdelend chose a model simple enough so that it does not inadvertently remove change points, as a pure linear trend would.  Thus, we use a GAM model of the form
...
Table~\ref{tab:homOrd} displays the average fitted efficiency as a function of $n$, outlier contamination, and \DIFdelbegin \DIFdel{the }\DIFdelend homogenization method.  \DIFdelbegin \DIFdel{The first }\DIFdelend \DIFaddbegin \DIFadd{Only the most promising methods from  Figure~\ref{fig:homEfficiency} are displayed here, and the first set of columns are the estimated efficiencies when the seasonality is not removed.  The second set of columns shows the increase in fitted efficiency when the seasonality is first removed with Equation~(\ref{eq:season}).   The first }\DIFaddend number indicates the fitted efficiency, averaged across all station and pressure level combinations, and the number in parentheses indicates the proportion of station and pressure level combinations where this model attains the highest fitted efficiency.  The robust SNHT with seasonal trend removed is the superior model in almost all scenarios.  However, the traditional SNHT is occasionally better when the outlier contamination rate is small (0\% or 1\%).  The robust SNHT with a 730 day period performs slightly better on the 40 year datasets but not enough to warrant its use.  However, this suggests that it may be optimal if many years of data are available. \DIFaddbegin \DIFadd{Almost all of the methods are improved by detrending first, but the SNHT and robust SNHT are not nearly as affected by detrending.  The BinSeg method  is improved by over 20\% for small sample sizes with the gain decreasing as the outlier contamination increases.  }\DIFaddend Based on these results, we use the robust SNHT for the remainder of this paper with $N=365$ and seasonal trend removed.
