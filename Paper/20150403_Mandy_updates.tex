...
%DIF DEL 20150330_QC_Paper.tex   Tue Mar 31 04:38:34 2015
%DIF ADD 20150403_QC_Paper.tex   Fri Apr  3 07:27:56 2015
...
However, to our knowledge, no research has been done to date describing which type of error should be handled first when a dataset contains both types of errors.  We do a simulation study in which data is contaminated with both known random errors and with known change points so that we can  evaluate  (1) the performance of our robust SNHT compared to other modern alternatives; (2) the effect of strong seasonality on homogenization methods; and (3) the sequence in which different quality control algorithms are applied.  We henceforth refer to the choice of performing random error detection or systematic error detection first  as ``the sequence of the \DIFdelbegin \DIFdel{quality control method}\DIFdelend \DIFaddbegin \DIFadd{methods}\DIFaddend '' or simply ``the sequence.''  \DIFdelbegin \DIFdel{In Section 2, we discuss }\DIFdelend \DIFaddbegin \DIFadd{We first describe }\DIFaddend the details of our data simulation and contamination.  \DIFdelbegin \DIFdel{Section 3 evaluates }\DIFdelend \DIFaddbegin \DIFadd{Then, }\DIFaddend the performance of the homogenization algorithms \DIFdelbegin \DIFdel{we use, and Section 4 gives }\DIFdelend \DIFaddbegin \DIFadd{are evaluated, and }\DIFaddend the results from the sequencing study \DIFdelbegin \DIFdel{.  In Section 5, we present }\DIFdelend \DIFaddbegin \DIFadd{are given.  Finally, we conclude with }\DIFaddend a case study of this method applied to a real dataset \DIFdelbegin \DIFdel{, and some conclusions are offered in Section 6.  }\textbf{%DIFDELCMD < [%%%
\DIFdel{Note--for IJoC submission, remove the references to section numbers in this paragraph.}%DIFDELCMD < ]%%%
}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{and offer some overall recommendations.  
}\DIFaddend 

%DIF >  In Section 2, we discuss the details of our data simulation and contamination.  Section 3 evaluates the performance of the homogenization algorithms we use, and Section 4 gives the results from the sequencing study.  In Section 5, we present a case study of this method applied to a real dataset, and some conclusions are offered in Section 6.  
\DIFaddbegin 

\DIFaddend \section{Simulation Method}
...
\begin{equation}
	...
	\label{eq:cost}
\end{equation}
where $\tau_i$ is the \DIFaddbegin \DIFadd{time that the }\DIFaddend $i$th change point \DIFaddbegin \DIFadd{occurs}\DIFaddend ; $m$ is the number of change points; $\mathcal{C}$ is a cost function; $y_{(\tau_{i-1}+1):\tau_i}$ is the observed data between the $(i-1)$ and $i$th change point; and $\beta f(m)$ is a penalty term on the number of change points to prevent overfitting \cite{killick12}.  Note that, for notational convenience, \DIFdelbegin \DIFdel{$\tau_{m+1}$ }\DIFdelend \DIFaddbegin \DIFadd{$y_{\tau_{m+1}}$ }\DIFaddend is defined to be the last observation.  Often, $\mathcal{C}$ is chosen to be twice the negative log likelihood, and $f(\cdot)$ is linear.
...
\ni is computed where $\bar{X}_i$ is the mean of $\bar{X}_{L,i}$ and $\bar{X}_{R,i}$, and $s_i$ is the estimated standard deviation over the $N$ days prior and $N$ days following observation $i$.  If there are not $N$ observations both before and after the current observation, no test is performed.  If the largest $T_i$ exceeds some threshold at time $i=i^*$, we conclude that a change point occurred at time $i^*$, and we adjust all observations after time $i^*$ by $\bar{X}_{L,i^*}-\bar{X}_{R,i^*}$. \DIFaddbegin \DIFadd{This homogenizes the time series to the older data, which we do here in simulations for convenience, but  in practice, it is generally preferable to homogenize to the most recent data, as it is considered to be more reliable \mbox{%DIFAUXCMD
\cite{domonkos13}
}%DIFAUXCMD
. %DIF > Section 2.3
 }\DIFaddend Homogenization now proceeds iteratively.  $T_i$ is recomputed for all $i$ that are sufficiently far away from the current change points, $i\in\{1,\ldots,n\} \setminus \{i^*-k, \ldots, i^*+k\}$, and the test is performed again until no $T_i$ exceed the threshold, and we use $k=N$.  
\DIFdelbegin \DIFdel{Note that in practice, it is generally preferable to homogenize to the most recent data, as that data is considered to be more reliable, and some follow this convention \mbox{%DIFAUXCMD
\cite{domonkos13}
}%DIFAUXCMD
. %DIF < Section 2.3
}\DIFdelend We note that the test statistic in \cite{haimberger07} differs from Equation~(\ref{eq:Hom}) in that a denominator of $s_i$ is used in place of $s_i^2$; however, we believe this to be a typographical error, as the statistic in \cite{haimberger07} is derived from \cite{alexandersson86}.  In \cite{alexandersson86}, $\bar{X}_{L,i}, \bar{X}_{R,i}$ and $\bar{X}_{i}$ are first standardized, and  this is equivalent to our formulation.
...
The threshold for determining when a change point has occurred is based on the sampling distribution of Equation~(\ref{eq:Hom}).  \DIFdelbegin \DIFdel{We first note that the SNHT algorithm generally makes the assumptions that the observations are independent and normal with some fixed mean. Under these assumptions,  }\DIFdelend \DIFaddbegin \DIFadd{The authors in \mbox{%DIFAUXCMD
\cite{haimberger07, alexandersson86,khaliq07}
}%DIFAUXCMD
generate the distribution of the SNHT with Monte Carlo techniques; however, this can be time-consuming, and we show here that the sampling distribution can be well-approximated with a parametric distribution. By replacing $\bar X_i$ in Equation~(\ref{eq:Hom}) with $(\bar{X}_{L,i}+\bar{X}_{R,i})/2$,  Equation~(\ref{eq:Hom})  can be rewritten as follows:
}\DIFaddend \begin{equation*}\DIFdelbegin \DIFdel{
	\frac{N}{2 \sigma^2}}%DIFDELCMD < \left( %%%
\DIFdel{(}%DIFDELCMD < \bar{X}%%%
\DIFdel{_{L,i}-}%DIFDELCMD < \bar{X}%%%
\DIFdel{_{R,i})^2}%DIFDELCMD < \right) %%%
\DIFdel{\sim \chi^2_1,
}\DIFdelend \DIFaddbegin \DIFadd{
%T_i= \left( \frac{\bar{X}_{L,i}-\bar{X}_{R,i}}{\sqrt{2}s_i/\sqrt{N}}\right)^2.
T_i= \frac{N\left( \bar{X}_{L,i}-\bar{X}_{R,i}\right)^2}{2s_i^2}.
}\DIFaddend \end{equation*}
\DIFdelbegin \DIFdel{\ni where $\sigma^2$ is the variance of the $X_i$.  If we replace $\sigma^2$ with $s_i^2$ we get our test statistic, }\DIFdelend \DIFaddbegin \DIFadd{Under the null hypothesis that the means on the left-hand }\DIFaddend and \DIFdelbegin \DIFdel{so }\DIFdelend \DIFaddbegin \DIFadd{right-hand sides of observation $i$ are the same, namely that $\bar X_{L,i}\sim N(\mu, \sigma^2/N)$ and  $\bar X_{R,i}\sim N(\mu, \sigma^2/N)$, the distribution of $(\bar{X}_{L,i}-\bar{X}_{R,i})$ is $N(0,2\sigma^2/N)$.  If the observations, $(X_1, \ldots, X_{i-1}, X_{i+1},\ldots,X_{2N})$, are independent and normally distributed, then clearly }\DIFaddend $T_i$ \DIFdelbegin \DIFdel{is approximately $\chi^2_1$.   
However, these assumptions do not all hold for }\DIFdelend \DIFaddbegin \DIFadd{follows a chi-squared distribution with 1 degree of freedom.   
}



\DIFadd{However,  }\DIFaddend radiosonde temperature data \DIFaddbegin \DIFadd{are not likely to be independent}\DIFaddend , as temperatures follow seasonal trends, which induce a strong dependence among the observations.  Thus, homogenization will likely be improved if the seasonal components of the data are removed.  We use a model-based approach to removing the seasonality; however, we  \DIFdelbegin \DIFdel{chose }\DIFdelend \DIFaddbegin \DIFadd{choose }\DIFaddend a model simple enough so that it does not inadvertently \DIFdelbegin \DIFdel{remove change points}\DIFdelend \DIFaddbegin \DIFadd{reduce the size of a change point}\DIFaddend , as a pure linear trend \DIFdelbegin \DIFdel{would}\DIFdelend \DIFaddbegin \DIFadd{could do}\DIFaddend .  Thus, we use a GAM model of the form
\begin{equation}\label{eq:season}
	t_i = \beta_0 + s_1(d_i) + \epsilon_i\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
where $t_i$ is the temperature at a given station and pressure level, $d_i$ is the day of the year of the $i$-th observation, $\beta_0$ is the intercept, $\epsilon_i$ is the error of the model, and $s_1$ is a cubic regression spline.   We then subtract this fit from the data, giving errors about an estimated seasonal mean.  These errors will no longer exhibit a seasonal trend, and  \DIFdelbegin \DIFdel{thus the assumptions of independent and mean-zero observations are much }\DIFdelend \DIFaddbegin \DIFadd{the assumption of independence becomes }\DIFaddend more reasonable.  This simple model \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend chosen because it  \DIFdelbegin \DIFdel{will leave }\DIFdelend \DIFaddbegin \DIFadd{leaves }\DIFaddend the numerator of the SNHT statistic unchanged \DIFdelbegin \DIFdel{, as }\DIFdelend \DIFaddbegin \DIFadd{since }\DIFaddend the same adjustment \DIFdelbegin \DIFdel{will be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend applied to the observations \DIFdelbegin \DIFdel{before $t_i$ as the observations after .  However, a more reasonable estimate for $s_i$ can now be obtained, and }\DIFdelend \DIFaddbegin \DIFadd{both before and after $t_i$.  In addition, with large sample sizes, we can assume even if the observations are not normally distributed, the sample means in the SNHT statistic will be normal by the Central Limit Theorem, thereby allowing us to approximate }\DIFaddend the distribution of the  SNHT statistic \DIFdelbegin \DIFdel{is more closely approximated by a chi-squared with one degree of freedom}\DIFdelend \DIFaddbegin \DIFadd{with the $\chi^2_1$}\DIFaddend .  We use the 99\% quantile of this distribution as our threshold.  \DIFdelbegin \DIFdel{Moreover, }\DIFdelend \DIFaddbegin \DIFadd{Since }\DIFaddend the SNHT statistic is applied many times to the same time series,  \DIFdelbegin \DIFdel{so }\DIFdelend we use the Benjamini-Hochberg adjustment for multiple testing of dependent tests to ensure that the family-wise Type I error rate remains below the 1\% threshold \cite{benjamini01}.
