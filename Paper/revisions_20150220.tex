...
%DIF DEL 20150220_QC_Paper.tex         Sat Feb 21 05:46:22 2015
%DIF ADD 20150220_QC_Paper_Mandy.tex   Sat Feb 21 05:43:50 2015
...
%DIF > 199 words
\ni  The historical radiosonde temperature archive, and indeed any large and lengthy observational dataset, must be quality controlled before it can be used properly.   Most research on quality control for such data focuses on the identification and removal of either systematic errors \DIFdelbegin \DIFdel{(homogenization) }\DIFdelend or random errors without considering an optimal process for treatment of both.  Additionally, little has been done to evaluate  homogenization methods \DIFaddbegin \DIFadd{that identify and correct systematic errors when }\DIFaddend applied to sub-daily data, and no research exists on using robust estimators in homogenization procedures.  In this paper, we simulate realistic radiosonde temperature data and contaminate it with both systematic and random errors.  We then evaluate (1) the performance of several homogenization algorithms \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with and without accounting for seasonality and }\DIFaddend (2) the sequence in which the random and systematic errors are identified and corrected.  In our simulations we find that the robust Standard Normal Homogeneity Test (SNHT) that we introduce performs better than the traditional SNHT, and it is better than several other modern alternatives.  Moreover, we find that systematic errors present in the data lead to poorer performance of random error removal algorithms, but the presence of random errors in the data is not as detrimental to homogenization algorithms.



%DIF < the effects of homogenizing data, via several homogenization algorithms, prior to or after removing errors and compare the resulting dataset with the ground truth. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{singlespacing}
\par\vfill\noindent
...
%DIF < : should answer "Why are you doing the study?  What's been done?  Why is what we're doing different than what's been done before?"
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Any large dataset whose observations reach far back in time may require treatment for both systematic and random errors. Datasets such as the International Surface Temperature Initiative (ISTI) global land surface databank \cite{rennie14} with over 32,000 stations, and the Integrated Global Radiosonde Archive (IGRA) housed at the National Climatic Data Center (NCDC) \cite{durre06} are examples of such large datasets. Systematic errors can occur when the station location changes; the area surrounding the station becomes urbanized; or the instrumentation is changed. Random errors can occur due to faulty data transmission; sporadic instrumentation problems; keystroke entries; or errors in data management. To illustrate, Figure~\ref{fig:BasicTS} plots the temperature recorded by radiosondes at the 50 mb pressure level at Station 70219 (Bethel, Alaska, USA); this time series appears to have random and possibly systematic errors.  It is important to treat both sources of errors in  large historical datasets as robustly and automatically as possible. In most published research, methods for handling systematic and random errors are \DIFdelbegin \DIFdel{treated }\DIFdelend \DIFaddbegin \DIFadd{presented }\DIFaddend separately, and opinions among climate and weather scientists differ in terms of which type of error should be handled first.  \DIFaddbegin \DIFadd{We use the term }\emph{\DIFadd{homogenization algorithms}} \DIFadd{to refer to the process of identifying and correcting systematic errors while }\emph{\DIFadd{quality control}} \DIFadd{(QC) methods are used to identify and remove random errors.   }\DIFaddend The purpose of this study is to shed light on the order in which systematic and random error methods should be applied to such large datasets when considering both sources of error simultaneously. In addition, robust estimators in homogenization algorithms when random errors are present have not yet been considered, so these are proposed and investigated as well.

In this paper, we  focus on the Upper Air Database (UADB) housed at the National Center for Atmospheric Research (NCAR). This archive differs from the IGRA archive in that it contains some different stations, and many of the records are older. Since the radiosonde data are the only \DIFaddbegin \DIFadd{direct }\DIFaddend measured values of the upper atmosphere, it is an important resource for studies in climate change \cite{elliott91, eskridge95} and for use as an input to global reanalysis datasets \cite{kalnay96, kanamitsu02}. Currently over 2,000 station locations exist, and atmospheric variables are collected at standard pressure levels as the radiosonde rises through the atmosphere. In large datasets such as these,  \DIFdelbegin \DIFdel{the }\DIFdelend error detection methods must be automated since the archives are so large that visual inspections of every station are not feasible.

Many methods have been developed to homogenize radiosonde data, but most are not tested on simulated data with known contamination errors \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{eskridge95, haimberger07, lanzante96, lanzante03, venema12}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{eskridge95, haimberger07, haimberger12, lanzante96, lanzante03, venema12}
}%DIFAUXCMD
}\DIFaddend .  However, a study was recently conducted by the European Cooperation in Science and Technology to compare many different homogenization methods \cite{venema12}.  A single large, realistic dataset with known change points was simulated, and then researchers were asked to test their homogenization algorithm on the dataset.  As the researchers did not have knowledge of the true change point locations, this experiment provided a way to compare the performance of  these  methods.  \DIFaddbegin \DIFadd{Another approach has been to generate realistic radiosonde temperatures using the Hadley Centre's atmospheric model HadAM3, add random Gaussian noise, and then introduce realistic systematic errors to compare homogenization methods \mbox{%DIFAUXCMD
\cite{titchner09, thorne11}
}%DIFAUXCMD
, but the computational requirements are so great that only a few  realizations can be simulated.
}\DIFaddend 

Most homogenization techniques are designed for monthly or annual time series, but radiosonde observations occur, on average, twice daily.  Some of these techniques rely on optimizing an objective function over all possible \DIFdelbegin \DIFdel{change point configurations }\DIFdelend \DIFaddbegin \DIFadd{points at which a systematic error could occur  }\DIFaddend \cite{killick12, li14, lu10, scott74}\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{.  Referred to in the statistical literature as }\emph{\DIFadd{change point detection}} \DIFadd{methods,   }\DIFaddend many of these approaches are too computationally expensive for daily data \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{and are not designed to work with nonstationary time series.  For example, a strong seasonal and daily trend is present in radiosonde temperatures, making them ill-suited for application of these methods.  }\DIFaddend Additionally, some methods may only locate proposed change points and may not correct for the difference in means, which is a necessary homogenization step.  In this paper, we compare the Standard Normal Homogeneity Test (SNHT) \cite{alexandersson86}, the PELT algorithm \cite{killick12}, and binary segmentation \cite{scott74} \DIFaddbegin \DIFadd{when applied to data wherein a seasonal trend has been removed}\DIFaddend .  We also propose a robust version of the SNHT.

Automated random error detection methods for radiosonde temperature data have not been investigated as thoroughly \cite{durre06,lanzante96}.  Models such as \cite{ignaccolo14} for  the entire vertical column using pairs of locations could be adapted for random error detection, but  recently in \cite{bell14}, several random error detection methods for a given location and pressure level are proposed and evaluated \DIFdelbegin \DIFdel{via }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend simulated datasets.  The authors find that the optimal error detection algorithm requires two steps: first scanning for observations that are too many standard deviations from the global mean and secondly scanning for observations that are too many standard deviations from their local mean.  Robust estimators of mean and standard deviation are used in both cases to mitigate the influence of errors, and a robust, asymmetric estimate of standard deviation is introduced to account for skewness in temperature \DIFdelbegin \DIFdel{data}\DIFdelend \DIFaddbegin \DIFadd{distributions}\DIFaddend .
...
\end{equation}
\ni where $t_i$ is the temperature at a given station and pressure level; $h_i$, $d_i$\DIFaddbegin \DIFadd{, }\DIFaddend and $y_i$ are the hour, day, and year of the $i$-th observation, respectively; $\beta_0$ is the intercept; $\beta_1$ is the coefficient for the long term trend; and $s_1(\cdot)$ and $s_2(\cdot)$ are cubic regression splines.

Typically the error term, $\epsilon_i$, in Equation~(\ref{eq:GAM}) would be modeled as normal with some unknown variance, but the distribution of the error terms could be skewed or have heavier tails than a normal distribution \DIFaddbegin \DIFadd{as shown by \mbox{%DIFAUXCMD
\cite{bell14}
}%DIFAUXCMD
for radiosonde temperatures}\DIFaddend .  Thus, we use a skew-$t$ distribution for the errors of this model, which has 4 parameters, $\xi, \sigma, \alpha$, and $\nu$, which are useful in controlling the first four moments of the distribution \cite{azzalini03}.  This distribution is very flexible and can handle skewed and heavy-tailed data.

Additionally, we expect there to be temporal correlation in the error terms.  However, since we have already included hourly and seasonal terms in the model,  \DIFdelbegin \DIFdel{we expect }\DIFdelend most of this autocorrelation \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{is already }\DIFaddend explained, so an AR(1) time series model is sufficient to account for the remaining structure in the residuals.  This model assumes that each error term has some fixed correlation with the error one time step in the past, and thus can be estimated by simply computing the correlation between $t_i$ and $t_{i+1}$ when the observations are equally spaced.

However, for radiosonde data, observations are not \DIFaddbegin \DIFadd{guaranteed to be }\DIFaddend equally spaced in time.  Launches are scheduled globally at 0 and 12 UTC; however, many deviations from this pattern are observed, especially in the historic record.  Most observations are within an hour or two of the scheduled launches, but in some instances, no launches occur on a given day, and on others, more than two radiosondes are launched.   Thus, to estimate the lag-$h$  autocorrelation, ${\phi}(h)$, in hours, we must use only those observations that are $h$ time steps apart:
...
Radiosonde observations are collected over long periods of time, as long as 100 years for some stations, and therefore systematic changes in the mean temperature are not uncommon.  These errors can happen for one of many reasons \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{such as }\DIFaddend changes in instrumentation, relocation of a station, \DIFaddbegin \DIFadd{or }\DIFaddend post-processing of data\DIFdelbegin \DIFdel{, etc}\DIFdelend .  Methods which detect and/or correct these errors are referred to as homogenization algorithms, and many such techniques have been developed by the meteorological community \cite{alexandersson86, domonkos13, gruber08, haimberger07, lanzante03, li14, lu10, venema12}.  Many of the homogenization algorithms make use of metadata, which document changes in the data collection process and/or compare data from neighboring stations.  We do not evaluate such algorithms since we simulate data from one station and pressure level at a time.  In \cite{haimberger07}, SNHT is applied by combining both metadata and the ERA-40, but we use a simplified version that operates purely on the observed data.

In this section, we compare the abilities of four different homogenization algorithms to detect systematic errors when random errors are also present in the data.  We summarize the methods we investigate, namely Binary Segmentation (BinSeg) \cite{scott74}, Pruned Exact Linear Time (PELT) \cite{killick12}, SNHT, and a new robust SNHT.  \DIFdelbegin \DIFdel{Additionally}\DIFdelend \DIFaddbegin \DIFadd{In addition}\DIFaddend , we study the performance of these algorithms when the seasonal trend is \DIFaddbegin \DIFadd{present versus having been }\DIFaddend modeled and removed\DIFdelbegin \DIFdel{or simply ignored}\DIFdelend .  We simulate data as described in Section 2 and then introduce change points and random errors, and we evaluate the ability of the algorithms to detect the known change points. 
...
PELT is another algorithm for optimizing Equation (\ref{eq:cost}), but it computes the exact minimum.  It proceeds recursively as follows: first, the optimal number and location of change points is determined for \DIFdelbegin \DIFdel{observations 1 and 2 }\DIFdelend \DIFaddbegin \DIFadd{the first two observations  }\DIFaddend only.  The optimal number and location of change points for the first three observations is then determined using this information, and more generally the optimal number and location of change points for the first $k+1$ observations is determined by considering the optimal configurations for the first $2, 3, \ldots, k$ observations.  PELT is computationally efficient.  For our analysis, we used the BinSeg and PELT algorithms implemented in the \texttt{changepoint} package in R \cite{killick14}.
...
%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We note that the }\DIFaddend test statistic in \cite{haimberger07} differs from \DIFdelbegin \DIFdel{the one here }\DIFdelend \DIFaddbegin \DIFadd{Equation~(\ref{eq:Hom}) }\DIFaddend in that a denominator of $s_i$ is used in place of $s_i^2$\DIFdelbegin \DIFdel{.  However, we believe that to }\DIFdelend \DIFaddbegin \DIFadd{; however, this must  }\DIFaddend be a typographical error, as the statistic in \cite{haimberger07} is derived from \cite{alexandersson86}.  In \cite{alexandersson86}, $\bar{X}_{L,i}, \bar{X}_{R,i}$ and $\bar{X}_{i}$ are first standardized, and  \DIFdelbegin \DIFdel{thus }\DIFdelend this is equivalent to our formulation.
\DIFdelbegin \DIFdel{However, for comparison purposes, we homogenize the simulated datasets with both statistics.  For the statistic with $s_i$ in the denominator, a threshold of 100 is recommended, and we find that value to work well in our simulations.
}\DIFdelend 

\DIFdelbegin \DIFdel{Now, a different threshold for the statistic with $s_i^2$ in the denominator is required}\DIFdelend %DIF > The test statistic in \cite{haimberger07} differs from the one here in that a denominator of $s_i$ is used in place of $s_i^2$.  However, we believe that to be a typographical error, as the statistic in \cite{haimberger07} is derived from \cite{alexandersson86}.  In \cite{alexandersson86}, $\bar{X}_{L,i}, \bar{X}_{R,i}$ and $\bar{X}_{i}$ are first standardized, and thus this is equivalent to our formulation.  However, for comparison purposes, we homogenize the simulated datasets with both statistics.  For the statistic with $s_i$ in the denominator, a threshold of 100 is recommended, and we find that value to work well in our simulations.
\DIFaddbegin 

\DIFadd{The threshold for determining when a change point has occurred is based on the sampling distribution of Equation~(\ref{eq:Hom})}\DIFaddend .  We first note that the SNHT algorithms generally make the assumption that the data is \DIFaddbegin \DIFadd{independent and }\DIFaddend approximately normal with some fixed mean, and under this assumption, the statistic follows a chi-squared distribution with \DIFdelbegin \DIFdel{one degree of freedom}\textbf{%DIFDELCMD < [%%%
\DIFdel{Josh: I still need to check this reference, but I'm pretty sure the Alexandersson paper states this}%DIFDELCMD < ]%%%
}%DIFAUXCMD
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{two degrees of freedom, i.e., $T_i\sim \chi^2_2$. }\textbf{[\DIFadd{Josh--isn't this a $\chi^2$ with 2 df's?}]} \textbf{[\DIFadd{Josh: I still need to check this reference, but I'm pretty sure the Alexandersson paper states this}]}  \DIFaddend This assumption is unreasonable with radiosonde temperature data, as temperatures follow seasonal trends\DIFaddbegin \DIFadd{, which induce a strong dependence among the observations}\DIFaddend .  Thus, homogenization will likely be improved if the seasonal components of the data are removed.  To do this, we fit a GAM model to the temperature data using only time of year as a predictor. \DIFaddbegin \textbf{[\DIFadd{Josh--you need to be more specific about the model that is fitted here, as you described in your email to me and in our conversation this week.  Not only what the model is but why you chose that particular one.}]}  \DIFaddend We then subtract this fit from the data, giving errors about a seasonal mean.  This simple model was chosen because it will leave the SNHT statistic unchanged, as the same adjustment will be applied to both $\bar{X}_{L, i}$ and $\bar{X}_{R, i}$. \DIFaddbegin \textbf{[\DIFadd{Josh--this is contradictory to what you said before that removing the seasonality will improve the SNHT.}]}  \DIFaddend It is now reasonable to assume that the SNHT statistic is chi-squared with \DIFdelbegin \DIFdel{one degree }\DIFdelend \DIFaddbegin \textbf{\DIFadd{two}} \DIFadd{degrees }\DIFaddend of freedom, and we \DIFdelbegin \DIFdel{pick }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend the 99\% quantile of this distribution as our threshold.  Moreover, the SNHT statistic \DIFdelbegin \DIFdel{performs many dependent tests, and thus }\DIFdelend \DIFaddbegin \DIFadd{is applied many times to the same time series, so }\DIFaddend we use the \DIFdelbegin \DIFdel{Bonferroni-Hochberg }\DIFdelend \DIFaddbegin \DIFadd{Benjamini-Hochberg }\DIFaddend adjustment for multiple testing \DIFaddbegin \DIFadd{of dependent tests to ensure that the Type I error rate remains below the 1\% threshold \mbox{%DIFAUXCMD
\cite{benjamini01}
}%DIFAUXCMD
}\DIFaddend .

We \DIFdelbegin \DIFdel{propose an alternative estimator that replaces the }\DIFdelend \DIFaddbegin \DIFadd{now propose a robust version of the SNHT wherein we replace the }\DIFaddend means and standard deviation in Equation~(\ref{eq:Hom}) with the Huber M-estimator of the mean and standard deviation \cite{huber11}.  These robust estimators of center and spread are computed as follows:
\begin{enumerate}
...
\begin{enumerate}
	...
	\item Steps 1-3 are repeated 1,000 \DIFaddbegin \textbf{[\DIFadd{Josh--this is not the same as the ``500" stated in the sentence below.}]} \DIFaddend times for each climate zone and pressure level.
	\DIFaddbegin 

	\textbf{[\DIFadd{Josh--need to mention seasonal versus detrended data in these steps somewhere.}]}
...
Figure~\ref{fig:homEfficiency} depicts a boxplot of the efficiencies measured for each of the different algorithms across all 90,000 simulations\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, i.e.~ for }\DIFaddend 500 simulations \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend each of 3 pressure levels, 10 stations, 3 sample sizes, and 2 seasonal removal \DIFdelbegin \DIFdel{configurations) }\DIFdelend \DIFaddbegin \DIFadd{(detrended or not) configurations}\DIFaddend .  The robust version of the SNHT appears to achieve the best efficiency among all homogenization algorithms considered.  The BinSeg algorithms perform best when we force the algorithm to choose a small number of change points.  However, in practice, we will not know the true number of change points, and the BinSeg algorithm is very sensitive to this choice.  The PELT algorithm appears to perform best with a penalty of $n/2$, but its performance is  worse than the alternative algorithms. \DIFaddbegin \textbf{[\DIFadd{Josh--Say something about the effect of removing the seasonal trend versus not across the algorithms.}]}
\DIFaddend 

To further understand the performance of these algorithms \DIFdelbegin \DIFdel{, }\DIFdelend and to understand their sensitivity to different simulation parameters, we fit a logistic regression model to the simulation results.  The response variable is 1 if efficiency is positive and 0 otherwise, and the independent variables we use are the sample size $n$, the outlier contamination rate, the station, the pressure level, the homogenization algorithm, and if a seasonal adjustment was applied.  We fit 4 different logistic regression models: first one with main effects and then models with $k$-way interactions, where $k=2,3,4$.  Table~\ref{tab:homOrdDev} reports the deviance for each model.  As the deviance does not decrease substantially after $k$ increases beyond 2, we use the model with only 2-way interaction terms.

Table~\ref{tab:homOrd} displays the average fitted efficiency as a function of $n$, outlier contamination, and the homogenization method.  The first number indicates the fitted efficiency, averaged across all station and pressure level combinations, and the number in parentheses indicates the proportion of station and pressure level combinations where this model attains the highest fitted efficiency.  The robust SNHT with seasonal trend removed is the superior model in almost all scenarios.  However, the traditional SNHT is occasionally better when the outlier contamination rate is small (0\% or 1\%).  The robust SNHT with a 730 day period performs slightly better on the 40 year datasets \DIFdelbegin \DIFdel{, }\DIFdelend but not enough to warrant \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend use.  However, this suggests that it may be optimal if many years of data are available.  Based on these results, we use the robust SNHT for the remainder of this paper with $N=365$ and seasonal trend removed.
...
\begin{tabular}{cc|ccc|ccc}
  \hline
  Number & Outlier & & No & & & Seasonal & \\
of Years & Contamination & & Adjustment & & & Adjustment &\\
  \hline
 & & SNHT-365 &  robust-365 & robust-730 &  SNHT-365 & robust-365 & robust-730\\
  \hline
& 0\% & 95.7\% (3\%) & 97.5\% (3\%) & 96.5\% (0\%) & 98.0\% (3\%) & \textbf{98.7\% (90\%)} & 97.3\% (0\%) \\ 
& 1\% & 95.6\% (0\%) & 97.8\% (7\%) & 96.8\% (0\%) & 97.9\% (0\%) & \textbf{98.8\% (93\%)} & 97.4\% (0\%) \\ 
10 & 2\% & 95.5\% (0\%) & 98.1\% (10\%) & 97.0\% (0\%) & 97.7\% (0\%) & \textbf{98.9\% (90\%)} & 97.5\% (0\%) \\ 
& 5\% & 95.1\% (0\%) & 98.8\% (17\%) & 97.6\% (0\%) & 97.1\% (0\%) & \textbf{99.2\% (83\%)} & 97.7\% (0\%) \\ 
& 10\% &94.4\% (0\%) & 99.4\% (30\%) & 98.3\% (0\%) & 95.8\% (0\%) & \textbf{99.5\% (70\%)} & 98.0\% (0\%) \\ 
\DIFaddbeginFL \hline
\DIFaddendFL & 0\% & 96.0\% (0\%) & 97.6\% (3\%) & 97.2\% (7\%) & 98.0\% (0\%) & \textbf{98.7\% (90\%)} & 97.6\% (0\%) \\ 
& 1\% & 95.9\% (0\%) & 97.9\% (10\%) & 97.4\% (3\%) & 97.8\% (0\%) & \textbf{98.8\% (87\%)} & 97.7\% (0\%) \\ 
20 & 2\% & 95.7\% (0\%) & 98.2\% (17\%) & 97.6\% (0\%) & 97.6\% (0\%) & \textbf{98.9\% (83\%)} & 97.7\% (0\%) \\ 
& 5\% & 95.3\% (0\%) & 98.8\% (23\%) & 98.0\% (0\%) & 96.9\% (0\%) & \textbf{99.1\% (77\%)} & 97.9\% (0\%) \\ 
& 10\% & 94.5\% (0\%) & 99.4\% (30\%) & 98.6\% (0\%) & 95.4\% (0\%) & \textbf{99.5\% (70\%)} & 98.1\% (0\%) \\ 
\DIFaddbeginFL \hline
\DIFaddendFL & 0\% & 96.5\% (0\%) & 97.9\% (0\%) & 98.2\% (27\%) & 97.8\% (0\%) & \textbf{98.5\% (70\%)} & 98.1\% (3\%) \\ 
& 1\% & 96.3\% (0\%) & 98.1\% (3\%) & 98.3\% (27\%) & 97.6\% (0\%) & \textbf{98.6\% (70\%)} & 98.1\% (0\%) \\ 
40 & 2\% & 96.2\% (0\%) & 98.4\% (7\%) & 98.4\% (23\%) & 97.3\% (0\%) & \textbf{98.7\% (70\%)} & 98.1\% (0\%) \\ 
& 5\% & 95.7\% (0\%) & 98.9\% (17\%) & 98.6\% (13\%) & 96.5\% (0\%) & \textbf{99.0\% (70\%)} & 98.2\% (0\%) \\ 
& 10\% & 94.6\% (0\%) & \textbf{99.4\% (53\%)} & 99.0\% (0\%) & 94.5\% (0\%) & 99.3\% (47\%) & 98.3\% (0\%) \\ 
\hline
\end{tabular}
\caption{Fitted efficiency averaged over all station and pressure level combinations.  Numbers in parentheses indicate the percent of station and pressure level combinations where the given model obtained the highest fitted efficiency. Bolded numbers are the highest within each row.  Note that only 6 of the 38 models are considered here.  For brevity, models were included only if they attained the highest efficiency for some configuration. \DIFaddbeginFL \textbf{\DIFaddFL{Josh--as discussed on Wed, I think it would be informative to put the change in fitted efficiency in the second set of columns to highlight just how much the seasonal adjustment has really moved fitted efficiency.  Also, include one more method where the seasonality removal had a big effect, like the BinSeg2.  }}\DIFaddendFL }
\label{tab:homOrd}
\end{table}
\end{landscape}
...
\begin{figure}[h!]
	...
	\end{tabular}
	\caption{Temperature for Station 70219 plotted over time.  The \DIFdelbeginFL \DIFdelFL{second }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right-hand }\DIFaddendFL image is the same as the first but zoomed in to the last ten years to show more detail.}
	\label{fig:BasicTS}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{Boxplot of efficiency scores for the various homogenization algorithms.  Note that this graph is constrained to the efficiency range of \DIFdelbeginFL \DIFdelFL{$(-3,1)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(-2,1)$ }\DIFaddendFL in order to show more detail.  The SNHT and the robust SNHT perform substantially  better than their alternatives.}
...
\begin{figure}[h!]
	...
	\caption{This graph depicts the estimated probability from the efficiency logistic regression model.  Each line represents a different simulation configuration (number of years simulated and outlier contamination rates).  \DIFdelbeginFL \DIFdelFL{This shows that there }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{There }\DIFaddendFL is substantial variability among the different simulations\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL but  \DIFdelbeginFL \DIFdelFL{that }\DIFdelendFL the SNHT and robust SNHT models perform better than the alternatives in all simulated configurations\DIFaddbeginFL \DIFaddFL{, and the efficiency improves with sample size}\DIFaddendFL .}
	\label{fig:homFitEff}
\end{figure}
