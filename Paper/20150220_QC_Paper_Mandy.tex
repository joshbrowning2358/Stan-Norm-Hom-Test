...
The threshold for determining when a change point has occurred is based on the sampling distribution of Equation~(\ref{eq:Hom}).  We first note that the SNHT algorithms generally make the assumption that the data is independent and approximately normal with some fixed mean, and under this assumption, the statistic follows a chi-squared distribution with two degrees of freedom, i.e., $T_i\sim \chi^2_2$. \textbf{[Josh--isn't this a $\chi^2$ with 2 df's?]} \textbf{[Josh: I still need to check this reference, but I'm pretty sure the Alexandersson paper states this]}  This assumption is unreasonable with radiosonde temperature data, as temperatures follow seasonal trends, which induce a strong dependence among the observations.  Thus, homogenization will likely be improved if the seasonal components of the data are removed.  We use a model-based approach to removing the seasonality; however, we must chose a model simple enough so that it does not inadvertently remove change points, as a pure linear trend would.  Thus, we use a GAM model of the form
\begin{align}
	t_i &= \beta_0 + s_1(d_i) + \epsilon_i
\end{align}
where $t_i$ is the temperature at a given station and pressure level, $d_i$ is the day of the year of the $i$-th observation, $\beta_0$ is the intercept, $\epsilon_i$ is the error of the model, and $s_1$ is a cubic regression spline.   We then subtract this fit from the data, giving errors about an estimated seasonal mean.  These errors will no longer exhibit a seasonal trend, and thus the assumption of normality is much more reasonable.  This simple model was chosen because it will leave the numerator of the SNHT statistic unchanged, as the same adjustment will be applied to the observations before $t_i$ as the observations after.  However, a more reasonable estimate for $s_i$ can now be obtained, and it is now reasonable to assume that the resulting SNHT statistic is chi-squared with \textbf{two} degrees of freedom.  We use the 99\% quantile of this distribution as our threshold.  Moreover, the SNHT statistic is applied many times to the same time series, so we use the Benjamini-Hochberg adjustment for multiple testing of dependent tests to ensure that the Type I error rate remains below the 1\% threshold \cite{benjamini01}.
...
\begin{enumerate}
	...
	\item Apply each homogenization algorithm to the contaminated dataset.  There is a total of 19 different homogenization algorithms, and each is applied to both the original and detrended data; thus, we have 38 different models to compare.
	\item Store the efficiency of each of the homogenization algorithms.
	\item Steps 1-3 are repeated 500 times for each climate zone, pressure level, and sample size.
\end{enumerate}
...
\hline
& 0\% & 96.0\% (0\%) & 97.6\% (3\%) & 97.2\% (7\%) & 98.0\% (0\%) & \textbf{98.7\% (90\%)} & 97.6\% (0\%) \\ 
...
\hline
& 0\% & 96.5\% (0\%) & 97.9\% (0\%) & 98.2\% (27\%) & 97.8\% (0\%) & \textbf{98.5\% (70\%)} & 98.1\% (3\%) \\ 
