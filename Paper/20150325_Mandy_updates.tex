...
%DIF DEL 20150312_QC_Paper.tex   Mon Mar 23 07:10:13 2015
%DIF ADD 20150325_QC_Paper.tex   Wed Mar 25 19:25:11 2015
...
Most homogenization techniques are designed for monthly or annual time series, but radiosonde observations occur, on average, twice daily.  Some of these techniques rely on optimizing an objective function over all possible points at which a systematic error could occur  \cite{killick12, li14, lu10, scott74}.  Referred to in the statistical literature as \emph{change point detection} methods,   many of these approaches are too computationally expensive for daily data and are not designed to work with nonstationary time series.  For example, a strong seasonal and daily trend is present in radiosonde temperatures, making them ill-suited for application of these methods.  Additionally, some methods may only locate proposed change points and may not correct for the difference in means, which is a necessary homogenization step.  In this paper, we compare the Standard Normal Homogeneity Test (SNHT) \cite{alexandersson86}, the PELT algorithm \cite{killick12}, and binary segmentation \cite{scott74} when applied to data \DIFdelbegin \DIFdel{wherein }\DIFdelend \DIFaddbegin \DIFadd{with a seasonal trend and for which }\DIFaddend a seasonal trend has been removed.  We also propose a robust version of the SNHT.

Automated random error detection methods for radiosonde temperature data have not been investigated as thoroughly \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{durre06,lanzante96}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{durre06,durre08,lanzante96}
}%DIFAUXCMD
}\DIFaddend .  Models such as \cite{ignaccolo14} for  the entire vertical column using pairs of locations could be adapted for random error detection, but  recently  \DIFdelbegin \DIFdel{in \mbox{%DIFAUXCMD
\cite{bell14}
}%DIFAUXCMD
, }\DIFdelend several random error detection methods\DIFaddbegin \DIFadd{, corresponding the climatological check step of \mbox{%DIFAUXCMD
\cite{durre08}
}%DIFAUXCMD
, }\DIFaddend for a given location and pressure level are proposed and evaluated with simulated datasets \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bell14}
}%DIFAUXCMD
}\DIFaddend .    The authors find that the optimal error detection algorithm requires two steps: first scanning for observations that are too many standard deviations from the global mean and secondly scanning for observations that are too many standard deviations from their local mean.  Robust estimators of mean and standard deviation are used in both cases to mitigate the influence of errors, and a robust, asymmetric estimate of standard deviation is introduced to account for skewness in temperature distributions.

However, to our knowledge, no research has been done to date describing which type of error should be handled first when a dataset contains both types of errors.  We do a simulation study in which data is contaminated with both known random errors and with known change points so that we can  evaluate  \DIFaddbegin \DIFadd{(1) }\DIFaddend the performance of our robust SNHT \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{compared to other modern alternatives; (2) the effect of strong seasonality on homogenization methods; and (3) }\DIFaddend the sequence in which different quality control algorithms are applied.  We henceforth refer to the choice of performing random error detection or systematic error detection first  as ``the sequence of the quality control method'' or simply ``the sequence.''  In Section 2, we discuss the details of our data simulation and contamination.  Section 3 evaluates the performance of the homogenization algorithms we use, and Section 4 gives the results from the sequencing study.  In Section 5, we present a case study of this method applied to a real dataset, and some conclusions are offered in Section 6.  \DIFaddbegin \textbf{[\DIFadd{Note--for IJoC submission, remove the references to section numbers in this paragraph.}]}
\DIFaddend 
...
\begin{equation} \label{eq:ACF}
	\widehat{\phi}(h)=\frac{1}{\lvert \mathcal{P}_h\rvert} \sum\DIFdelbegin \DIFdel{_{(\widehat{\epsilon_i},\widehat{\epsilon_j}) \in \mathcal{P}_h} \frac{(\widehat{\epsilon_i}-\bar{\epsilon_i})(\widehat{\epsilon_j}-\bar{\epsilon_j})}{\sqrt{s_{\epsilon_i} s_{\epsilon_j}}}}\DIFdelend \DIFaddbegin \DIFadd{_{(\hat{\epsilon}_i,\hat{\epsilon}_j) \in \mathcal{P}_h} \frac{(\hat{\epsilon}_i-\bar{\epsilon})(\hat{\epsilon}_j-\bar{\epsilon})}{s^2_{\hat\epsilon}}}\DIFaddend ,
\end{equation}
\ni where $\mathcal{P}_h$ is the set of all pairs of residuals that are $h$ hours apart (or within some window)\DIFdelbegin \DIFdel{, and $\widehat{\epsilon_i}$ }\DIFdelend \DIFaddbegin \DIFadd{; $|\mathcal{P}_h|$ is the number of pairs of residuals in the set $\mathcal{P}_h$;  $\hat{\epsilon}_i$ }\DIFaddend is the observed residual from Equation~(\ref{eq:GAM})\DIFaddbegin \DIFadd{; $\bar\epsilon$ is the average of the residuals; and $s^2_{\hat\epsilon}$ is the standard deviation of the residuals}\DIFaddend .  For an AR(1) model, we need only estimate $\phi(\cdot)$ at $h=12$ hours, and we use a window of 5\% of 12 hours, or 0.6 hours.
...
\begin{enumerate}
	\item Fit Equation~(\ref{eq:GAM}) to \DIFaddbegin \DIFadd{observed }\DIFaddend radiosonde temperature data at a given location and pressure level.  Then, fit a skew-$t$ distribution to the error terms, and model the autocorrelation with Equation~(\ref{eq:ACF}).
	...
	\end{equation*}
	where $\epsilon_i$ is the simulated noise in the model at time $i$, and $\Delta_{i-1}$ is the time difference, in hours, between the $(i-1)$th and $i$th observation.  Note that the \DIFdelbegin \DIFdel{$k$}\DIFdelend \DIFaddbegin \DIFadd{$i$}\DIFaddend th term in this series will depend on all \DIFdelbegin \DIFdel{the previous $k-1$ }\DIFdelend \DIFaddbegin \DIFadd{of the previous $i-1$ }\DIFaddend values.  To ensure the correct correlation structure, we simulate 1,000 more values than needed and discard the first 1,000.
	\item Then, \DIFaddbegin \DIFadd{the simulated }\DIFaddend $\epsilon_i$ is added to $\hat{t}_i$ to construct a simulated series that is similar to real radiosonde temperatures.
...
The threshold for determining when a change point has occurred is based on the sampling distribution of Equation~(\ref{eq:Hom}).  We first note that the SNHT \DIFdelbegin \DIFdel{algorithms generally make the assumption that the data is }\DIFdelend \DIFaddbegin \DIFadd{algorithm generally makes the assumptions that the observations are }\DIFaddend independent and approximately normal with some fixed mean, \DIFdelbegin \DIFdel{and under this assumption, }\DIFdelend \DIFaddbegin \DIFadd{which imply that }\DIFaddend the statistic follows a chi-squared distribution with two degrees of freedom, i.e., $T_i\sim \chi^2_2$. \textbf{[\DIFdelbegin \DIFdel{Josh--isn't this a $\chi^2$ with 2 df's?}\DIFdelend \DIFaddbegin \DIFadd{Josh--triple-check based on the Alexandersson paper.}\DIFaddend ]}  \DIFdelbegin \textbf{%DIFDELCMD < [%%%
\DIFdel{Josh: I still need to check this reference, but I'm pretty sure the Alexandersson paper states this}%DIFDELCMD < ]%%%
}  %DIFAUXCMD
\DIFdel{This assumption is unreasonable with }\DIFdelend \DIFaddbegin \DIFadd{However, these assumptions do not all hold for }\DIFaddend radiosonde temperature data, as temperatures follow seasonal trends, which induce a strong dependence among the observations.  Thus, homogenization will likely be improved if the seasonal components of the data are removed.  We use a model-based approach to removing the seasonality; however, we  \DIFdelbegin \DIFdel{must }\DIFdelend chose a model simple enough so that it does not inadvertently remove change points, as a pure linear trend would.  Thus, we use a GAM model of the form
\DIFdelbegin \begin{eqnarray*}\DIFdel{
	t_i }&\DIFdel{= \beta_0 + s_1(d_i) + \epsilon_i
}\end{eqnarray*}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{eq:season}
	t_i = \beta_0 + s_1(d_i) + \epsilon_i
}\end{equation}
\DIFaddend where $t_i$ is the temperature at a given station and pressure level, $d_i$ is the day of the year of the $i$-th observation, $\beta_0$ is the intercept, $\epsilon_i$ is the error of the model, and $s_1$ is a cubic regression spline.   We then subtract this fit from the data, giving errors about an estimated seasonal mean.  These errors will no longer exhibit a seasonal trend, and thus the \DIFdelbegin \DIFdel{assumption of normality is }\DIFdelend \DIFaddbegin \DIFadd{assumptions of independent and mean-zero observations are }\DIFaddend much more reasonable.  This simple model was chosen because it will leave the numerator of the SNHT statistic unchanged, as the same adjustment will be applied to the observations before $t_i$ as the observations after.  However, a more reasonable estimate for $s_i$ can now be obtained, and \DIFdelbegin \DIFdel{it is now reasonable to assume that the resulting }\DIFdelend \DIFaddbegin \DIFadd{the distribution of the  }\DIFaddend SNHT statistic is \DIFaddbegin \DIFadd{more closely approximated by a }\DIFaddend chi-squared with \DIFdelbegin \textbf{\DIFdel{two}} %DIFAUXCMD
\DIFdelend \DIFaddbegin {\DIFadd{two}} \DIFaddend degrees of freedom.  We use the 99\% quantile of this distribution as our threshold.  Moreover, the SNHT statistic is applied many times to the same time series, so we use the Benjamini-Hochberg adjustment for multiple testing of dependent tests to ensure that the \DIFaddbegin \DIFadd{family-wise }\DIFaddend Type I error rate remains below the 1\% threshold \cite{benjamini01}.
...
\begin{enumerate}
	...
	\item Apply each homogenization algorithm to the contaminated dataset.  \DIFdelbegin \DIFdel{There is }\DIFdelend \DIFaddbegin \DIFadd{Counting all of the homogenization algorithms along with each one's unique tuning parameters, we apply   }\DIFaddend a total of 19 different homogenization algorithms\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. Since }\DIFaddend each is applied to both the original and detrended data\DIFdelbegin \DIFdel{; thus}\DIFdelend , we have 38 different \DIFdelbegin \DIFdel{models }\DIFdelend \DIFaddbegin \DIFadd{methods }\DIFaddend to compare.
	\item Store the efficiency of each \DIFdelbegin \DIFdel{of the homogenization algorithms}\DIFdelend \DIFaddbegin \DIFadd{method}\DIFaddend .
	\item Steps 1-3 are repeated 500 times for each \DIFaddbegin \DIFadd{combination of }\DIFaddend climate zone, pressure level, and sample size.
\end{enumerate}
...
We apply four different sequencings of homogenization and random error identification to the data: homogenization followed by \DIFaddbegin \DIFadd{random }\DIFaddend error detection; \DIFaddbegin \DIFadd{random }\DIFaddend error detection followed by homogenization; homogenization followed by \DIFaddbegin \DIFadd{random }\DIFaddend error detection followed by homogenization; and \DIFaddbegin \DIFadd{random }\DIFaddend error detection followed by homogenization followed by \DIFaddbegin \DIFadd{random }\DIFaddend error detection.  We refer to these approaches as ``Sys-Ran,'' ``Ran-Sys,'' ``Sys-Ran-Sys,'' and ``Ran-Sys-Ran,'' respectively.

We hypothesize that \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{some }\DIFaddend random errors will not be detected if the data is not homogenized and that the homogenization procedure will not perform as well if random errors are not first removed.  For these reasons, we included the two additional methods ``Sys-Ran-Sys'' and ``Ran-Sys-Ran''.  In both of these approaches, a homogenization procedure is performed after random error detection.  Likewise, a random error detection will be performed after a homogenization algorithm as well.
...
\begin{enumerate}
	...
	\item Steps 1-3 are repeated 1,000 times for each climate zone\DIFdelbegin \DIFdel{and pressure level}\DIFdelend \DIFaddbegin \DIFadd{,  pressure level, and sample size}\DIFaddend .
\end{enumerate}
...
For the TPR model, we find that there is no significant difference between the sequencings ``Ran-Sys-Ran,'' ``Sys-Ran,'' and ``Sys-Ran-Sys.''  Table~\ref{tab:fitTPR} shows the fitted TPR averaged across all stations and pressure levels, and the number in parentheses shows the percent of \DIFdelbegin \DIFdel{the time }\DIFdelend \DIFaddbegin \DIFadd{instances }\DIFaddend when that model attains the highest fitted TPR.  Additionally, these effects are plotted in Figure~\ref{fig:fitTPR}.  The table shows that the ``Ran-Sys'' sequencing performs best when the outlier contamination rate is small and when the sample size is relatively small.  The other sequences perform  \DIFdelbegin \DIFdel{quite similarly }\DIFdelend \DIFaddbegin \DIFadd{similarly and do much better than the ``Ran-Sys" sequence when the sample size is medium to large and as the outlier contamination rate increases}\DIFaddend .  

For the FPR model, we \DIFdelbegin \DIFdel{found no significant difference between }\DIFdelend \DIFaddbegin \DIFadd{did not find a substantial difference among }\DIFaddend the four sequencings.  Table~\ref{tab:fitFPR} shows the fitted false positive rates averaged across all stations and pressure levels\DIFdelbegin \DIFdel{.  Additionally, }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend these effects are plotted in Figure~\ref{fig:fitFPR}.  In almost all simulations, ``Ran-Sys-Ran'' attained the lowest FPR; however, FPR is quite low across all models.  Due to this fact, and the conclusions from the TPR model, we recommend one of the sequencings ``Ran-Sys-Ran'', ``Sys-Ran'', or ``Sys-Ran-Sys'' if the end goal is to maximize FPR and minimize TPR.

Lastly, results from fitting the efficiency model are shown in Table~\ref{tab:fitEff}.  The largest fitted efficiency is almost always obtained with the sequence ``Sys-Ran''.  However, as shown in Figure~\ref{fig:fitEffOrd}, the difference among the four sequencings is not \DIFdelbegin \DIFdel{statistically significant}\DIFdelend \DIFaddbegin \DIFadd{substantial}\DIFaddend .  Thus, we conclude that \DIFaddbegin \DIFadd{when using the robust homogenization algorithm, }\DIFaddend the sequencing chosen does not have a large effect on the efficiency of the final homogenized data.
...
In \cite{haimberger07}, station 70219 (Bethel, Alaska, USA) is analyzed for change points.  We perform a similar analysis, although we use raw radiosonde temperatures rather than the difference between the observed temperature  and a reanalysis.  We apply the robust SNHT to this time series followed by the outlier detection algorithm described in Section~\ref{sec:ranErr}.  As the dataset contains 53 years of data, we use a window of 2 years\DIFaddbegin \DIFadd{, $N=730$}\DIFaddend .

\DIFaddbegin \textbf{[\DIFadd{Josh--you need to double-check all of these numbers in this paragraph.  They are the same as a prior version, but the case study figures have changed.}]} \DIFaddend The algorithm detected a total of 15 change points and 19 outliers.  The detected change points have mean shifts ranging from 1.01$^\circ$C to 2.80$^\circ$C, and the average is 1.55$^\circ$C. \DIFaddbegin \textbf{[\DIFadd{Josh--are all of the mean shifts positive?  No, not according to the plots.  So, instead put the real range, not just the absolute values here.  You can report that the average absolute shift is $x$, whatever it is.}]}  \DIFaddend The global random error detection algorithm identified 12 errors, with test statistics ranging from 6.91 to 28.51 and averaging 15.57.  The windowed random error detection algorithm detected an additional 7 errors, with test statistics ranging from 5.63 to 6.95 and averaging 6.12. Figure~\ref{fig:caseStudy} depicts a plot of the time series before and after the quality control algorithm has been applied.  Only a subset of the data is plotted in the middle panel in order to show the effects of homogenization more clearly.  In the 10 year period shown, three change points are detected as indicated by the dashed vertical lines.  The detected change points shift the mean of the process, and the corrections made in the quality controlled dataset seem to improve the homogeneity of the final product. \DIFaddbegin \textbf{[\DIFadd{Josh--how do your results compare the the results found in \mbox{%DIFAUXCMD
\cite{haimberger07}
}%DIFAUXCMD
?  He used the same data, so check back.}]}
\DIFaddend 

\section{Conclusion}

In this study we evaluate several different homogenization techniques, and we find that the robust SNHT method with seasonality removed performs \DIFdelbegin \DIFdel{well}\DIFdelend \DIFaddbegin \DIFadd{the best among those considered}\DIFaddend .  It attains a high efficiency, indicating that this method is reasonably effective at returning the data to its uncontaminated state.  It attains higher efficiencies then the BinSeg and PELT algorithms, and it outperforms the non-robust SNHT even when the outlier contamination rate is small.  The optimal period may be a function of the size of the dataset, but the 365 day period is optimal for datasets with 40 years or less.

We also evaluate the effect that the sequence in which the random error detection and homogenization algorithms are applied have on the final performance of the overall quality control routine.  We find that failing to remove systematic errors before searching for random errors leads to a much lower true positive rate of the error removal algorithm in most cases.  However,  the removal of random errors first does not have a large influence on the detection of systematic errors \DIFaddbegin \DIFadd{as long as a homogenization method that is robust to random errors is applied}\DIFaddend .

In the absence of metadata regarding a station's history, we recommend performing data homogenization first \DIFaddbegin \DIFadd{with the robust SNHT }\DIFaddend followed by random error detection.  This two step procedure performs significantly better than its reversal, and it performs similarly to three step procedures.  The three step procedures do not perform significantly better than ``Sys-Ran.''  Since the three step procedures are more computationally expensive, especially given the size of the radiosonde archive, we recommend against their use.  Our recommended approach can be applied throughout the archive with minimal human intervention;   however, gains in random and systematic error detection may be realized if metadata, multiple pressure levels, or multiple station locations are combined, as in \cite{ignaccolo14}.  More work is needed to determine how to best handle both types of errors  when such additional information is incorporated.   
...
\DIFaddbeginFL \begin{footnotesize}
	...
	\begin{tabular}{cc|cccc|cccc}
		\hline
		\DIFdelbeginFL \DIFdelFL{Number }\DIFdelendFL \DIFaddbeginFL \# \DIFaddFL{of }\DIFaddendFL & Outlier & \multicolumn{4}{c}{Estimated Efficiency} & \DIFdelbeginFL %DIFDELCMD < \multicolumn{4}{c}{Increase in Efficiency with Seasonal Model} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{4}{c}{Change in Efficiency with Seasonal Model} \DIFaddendFL \\ 
		...
		\hline
		 ...
		 \DIFaddbeginFL \hline
		 \DIFaddendFL & 0\% & 96.5\% (0\%) & 97.9\% (0\%) & 98.2\% (27\%) & 81.7\% (0\%) & 1.3\% (0\%) & \textbf{0.6\% (70\%)} & -0.1\% (3\%) & 12.2\% (0\%) \\ 
		...
		\hline
	\end{tabular}
\caption{\DIFdelbeginFL \DIFdelFL{Fitted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The first set of columns show the fitted }\DIFaddendFL efficiency averaged over all station and pressure level combinations \DIFaddbeginFL \DIFaddFL{when the seasonality is not removed}\DIFaddendFL .  Numbers in parentheses indicate the percent of station and pressure level combinations where the given model obtained the highest fitted efficiency.  \DIFaddbeginFL \DIFaddFL{The second set of columns show the change in efficiency when the seasonality is fit and removed first.    }\DIFaddendFL Bolded numbers are the \DIFdelbeginFL \DIFdelFL{highest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{best }\DIFaddendFL within each row.  \DIFdelbeginFL \DIFdelFL{Note that only 6 of the 38 models are considered here.  }\DIFdelendFL For brevity, \DIFdelbeginFL \DIFdelFL{models were included }\DIFdelendFL only \DIFdelbeginFL \DIFdelFL{if they attained }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{those models with }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{highest efficiency for some configuration}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{best efficiencies are included here}\DIFaddendFL .  }
\label{tab:homOrd}
\DIFaddbeginFL \end{footnotesize}
...
\begin{figure}
	...
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.45\textwidth]{35121_simulated_3.png}
%DIFDELCMD < 	\includegraphics[width=.45\textwidth]{35121_contaminated_3.png}\\
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > \includegraphics[width=.45\textwidth]{35121_simulated_3.png}
	%DIF > \includegraphics[width=.45\textwidth]{35121_contaminated_3.png}\\
...
\begin{figure}[h!]
	...
	\caption{This graph depicts the estimated probability \DIFaddbeginFL \DIFaddFL{that the efficiency exceeds one }\DIFaddendFL from the efficiency logistic regression model.  Each line represents a different simulation configuration (number of years simulated and outlier contamination rates).  There is substantial variability among the different simulations, but  the SNHT and robust SNHT models perform better than the alternatives in all simulated configurations, and the efficiency improves with sample size.}
	\label{fig:homFitEff}
\end{figure}
...
\begin{figure}
	...
	\caption{Time series plot of the radiosonde temperature data at station 70219 (Bethel, Alaska, USA).  The top plot shows the data prior to the quality control algorithm, and the other two show the data after \DIFaddbeginFL \DIFaddFL{for a subset (middle) and the entire record (bottom)}\DIFaddendFL .  Dashed \DIFaddbeginFL \DIFaddFL{vertical }\DIFaddendFL lines indicate detected change points, numbers \DIFaddbeginFL \DIFaddFL{in the top panel }\DIFaddendFL indicate the magnitude of the corresponding change point, and dots indicate detected random errors.}
	\label{fig:caseStudy}
\end{figure}
